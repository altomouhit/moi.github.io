...
function loadMailData(e) {
	 Array.from(e).forEach(function(e, t) {
        ...
        // '<li class="' + a + '"><div class="col-mail col-mail-1"><div class="form-check checkbox-wrapper-mail fs-14">'+
        // '<input class="form-check-input" type="checkbox" value="' + e.id + '" id="checkbox-' + e.id + '">'+
        // '<label class="form-check-label" for="checkbox-' + e.id + '"></label></div>'+
        // '<button type="button" class="btn avatar-xs p-0 favourite-btn fs-15 ' + l + '"><i class="ri-star-fill"></i></button>'+
        // '<a href="javascript: void(0);" class="title"><span class="title-name">' + e.name + "</span> " + c + '</a></div>'+
        // '<div class="col-mail col-mail-2"><a href="javascript: void(0);" class="subject"><span class="subject-title">' + e.title + '</span>' +
        // 'Re<span class="teaser">' + e.description + '</span></a>'+
        // '<div class="date">' + e.date + "</div></div>'</li>", 
		'<tr class="' + a + '">'+
		'<td class="">'+
...
function loadSocialMailData(e) {
	Array.from(e).forEach(function(e, t) {
        ...
        '<div class="col-mail col-mail-2"><a href="javascript: void(0);" class="subject"><span class="subject-title">' + e.title + '</span>'+
        ' Re <span class="teaser">' + e.description + '</span></a>'+
        '<div class="date">' + e.callDate + "</div></div></li>",
        emailDetailShow(), emailDetailChange()
	})
}
...
function checkBoxAll() {
	...
	var e = document.querySelectorAll(".tab-pane.show .checkbox-wrapper-mail input");
	...
	Array.from(e).forEach(function(e) {
		e.addEventListener("click", function(e) {
			...
			a.checked = 0 < l, a.indeterminate = 0 < l && l < t.length, e.target.closest("li").classList.contains("active")
		})
...
getJSON("mail-list.init.json", function(e, t) {
	null !== e ? console.log("Something went wrong: " + e) :
	 (allmaillist = t[0].primary, socialmaillist = t[0].social, promotionsmaillist = t[0].promotions, 
	...
	Array.from(document.querySelectorAll(".message-list tr")).forEach(function(c) {
		c.addEventListener("click", function() {
			var e = (document.querySelector(".remove-mail").setAttribute("data-remove-id", e), 
				...
				b = (document.querySelector(".call-date").innerHTML = e, 
				c.querySelector(".title-date").innerHTML),
